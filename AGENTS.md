...
- The site uses per-page-category background images: `green_side.jpg` for Science & Tech pages, `yellow_side.jpg` for Art & Design and Pieces (`/pieces`) pages, `livia.jpg` for neutral/home pages. Site navigation: Home (`/`), Biography (`/biography`), Art & Design (`/art-design`), Science & Tech (`/science-tech`), Pieces (`/pieces`); no external links in top-level nav. Home uses the portrait background, a prominent native “Enter Livistone” link between the portrait’s chin and embedded name, and the edge tool rails (no duplicate floating arc nav). `livistone_entry()` links to https://livistone.liviazaharia.com in the same tab; its responsive styles in `assets/bubbles.css` use an independent type scale, opaque background, and keyboard focus ring. Position follows the portrait’s `cover` scaling and `center 18%` crop; keep the button slim so it does not cover the image lettering. The centered button fits its text up to the viewport clearance. The full invitation uses Manrope throughout, with “Enter Livistone” slightly larger, and wraps on phones; hide only the decorative arrow at narrow widths, never the invitation text. Art & Design and Science & Tech use folder-based tabs auto-discovered from `content/art-design/` and `content/science-tech/`; bottom nav highlights the active page via `rx.State.router.page.path`.
